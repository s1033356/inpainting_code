...
class StructureFlow():
    ...
    def train(self):
        train_writer = self.obtain_log(self.config, mode = 'TRAIN')
        val_writer = self.obtain_log(self.config, mode = 'VAL')
        ...
        val_dataset = Dataset(self.config.DATA_VAL_GT, self.config.DATA_VAL_STRUCTURE, 
                              self.config, self.config.DATA_VAL_MASK)
        sample_dataset = Dataset(self.config.DATA_SAMPLE_GT, self.config.DATA_SAMPLE_STRUCTURE, self.config, self.config.DATA_SAMPLE_MASK)
        sample_iterator = sample_dataset.create_iterator(self.config.SAMPLE_SIZE)
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                if model == 1:
                    ...
                    # backward
                    self.flow_model.structure_backward()                 
                    iterations = self.flow_model.iterations
                # flow model
                elif model == 2:
    ...
    def eval(self, writer=None):
        ...
        with torch.no_grad(): 
            ...
            for j, items in enumerate(islice(val_loader, 50)):
                ...
                log = [("it", iteration), ] + log
                progbar.add(len(inputs), values=log)
        for lst_num in range(logs_list):
            
            

        avg_psnr = np.average(psnr_list)
        avg_ssim = np.average(ssim_list)
        avg_l1 = np.average(l1_list)

        # print(logs)      
        # if writer is not None:
        writer.add_scalar('eval_psnr', avg_psnr)
        writer.add_scalar('eval_ssim', avg_ssim)
        writer.add_scalar('eval_l1', avg_l1)
        #write_loss(logs, writer)
    ...
    def obtain_log(self, config, mode):
        log_dir = os.path.join(config.PATH, config.NAME, self.stage_name+'_log', mode)
    ...
    def write_loss(self, logs, writer):
        iteration = [x[1] for x in logs if x[0]=='iter']
        for x in logs:
            if x[0].startswith('l_'):
                writer.add_scalar(x[0], x[1], iteration[-1])
